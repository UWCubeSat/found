...
Eigen::VectorXd TLS(Eigen::MatrixXd data) {
    ...
    // Since the input matrix should be thin and tall, the last column of V transpose
    ...
    // We do head(rows - 1) because the function asks for the number of coefficients in the vector
    ...
    // the result dots to the outputs as we expect.
