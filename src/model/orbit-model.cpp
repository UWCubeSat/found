...
std::pair<Vec3, Vec3> OrbitModel::propagate_r_v(const Vec3& r0, const Vec3& v0, double dt, double t_total) const {
    ...
    auto [t_vals, r_vals, v_vals] = rk4(t0, r0,
        [this](double t, const Vec3& y, const Vec3& v, double mu) {
            return this->first_deriv(t, y, v, mu);
        },
        [this](double t, const Vec3& y, const Vec3& v, double mu) {
            return this->second_deriv(t, y, v, mu);
        },
        dt, n, v0, mu_earth_km3s2
    );
...
std::vector<double> OrbitModel::get_orbital_elements(const Vec3& r, const Vec3& v) const {
    return state_vector_to_elements(r, v, mu_earth_km3s2);
}

Vec3 OrbitModel::first_deriv(double /*t*/, const Vec3& /*y*/, const Vec3& v, double /*mu*/) const {
    return v;
}

Vec3 OrbitModel::second_deriv(double /*t*/, const Vec3& y, const Vec3& /*v*/, double mu) const {
