...
void printDataFile(const DataFile& data, const std::string& label) {
    ...
    for (size_t i = 0; i < data.positions.size(); ++i) {
        std::cout << "Position " << i + 1 << ": (" 
                  ...
                  << data.positions[i].position.y << ", " 
                  << data.positions[i].position.z << ")" << std::endl;
    }
    std::cout << "====================" << std::endl;
}
...
void test_basic() {
    ...
    for (size_t i = 0; i < originalData.positions.size(); ++i) {
        assert(originalData.positions[i].position.x == loadedData.positions[i].position.x);
        assert(originalData.positions[i].position.y == loadedData.positions[i].position.y);
        assert(originalData.positions[i].position.z == loadedData.positions[i].position.z);
    }
